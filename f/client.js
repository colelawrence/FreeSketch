 ...
 * Compiled on:  12/26/2012 11:08:47 PM
 * Compiled by: Isaac\Cole
 * Source: C:\Users\Cole\Documents\My Web Sites\FreeSketch\f\client.coffee      
 * -------------------------------------------------------------------------*/
...
getId = function() {
  if (client_id === null) client_id = makeID();
  return client_id;
};
...
openNotice = function(evt) {
  if (!inputOpen) {
    openInput();
    inputOpen = true;
    if (evt.keyCode === 32) $('#inputBox').val('');
  }
  if (evt.keyCode === 13) {
    sendAndClear();
    return closeInput();
...
closeInput = function() {
  ...
  removeCanvasSaver();
  if (colorOutOfDate) updateColor();
  if (sizeOutOfDate) updateSize();
  $('#inputBox').val('');
...
getPencil = function(cid) {
  var pencil, _i, _len;
  for (_i = 0, _len = pencilBox.length; _i < _len; _i++) {
    pencil = pencilBox[_i];
    if (pencil.cid === cid) return pencil;
  }
...
drawSData = function(d) {
  ...
  if (pencil === null || typeof pencil === 'undefined') return;
  x2 = d[1];
  y2 = d[2];
...
getCookie = function(cookieKey) {
  var cookie, cookieKV, pageCookies, _i, _len;
  pageCookies = document.cookie.split(";");
  for (_i = 0, _len = pageCookies.length; _i < _len; _i++) {
    cookie = pageCookies[_i];
    cookieKV = cookie.split("=");
    console.log(cookieKV);
    if (cookieKV[0] === cookieKey) return unescape(cookieKV[1]);
  }
  return null;
};
...
checkUsername = function() {
  ...
  if (username === null || username === "") {
    ...
    if (username !== null && username !== "") setCookie("username", username, 21);
  }
...
Game = (function() {
  ...
  Game.prototype.draw = function(x2, y2) {
    ...
    if (this.pencilOutOfDate) getConnection().sendPencil(this.uSize, this.uColor);
  ...
  Game.prototype.drawStep2 = function(aX, aY, bX, bY) {
    if (aX === 0 && aY === 0) return getConnection().sendPutdown();
    this.ctx.beginPath();
...
Connection = (function() {
  ...
  Connection.prototype.Receive = function(msg) {
    var dA;
    dA = msg.data.split(":");
    if (dA[0] === 'b') return changePencil(dA);
    if (dA[0] === 'p') return pickupAPencil(dA[1]);
    if (dA[0] === 'd') return putdownAPencil(dA[1]);
    if (dA[0] === 'm') return messageData(dA);
    if (dA[0] === 's') return serverMessage(dA);
    if (dA[0] === 'u') return userConnection(dA);
    if (dA[0] === 'x') return removePencil(dA[1]);
    return drawSData(dA);
  };
